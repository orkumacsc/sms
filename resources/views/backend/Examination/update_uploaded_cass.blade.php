...
@php
	...
	foreach ($Assessments as $cass) {
		$cassMax[$cass->ass_type_id] = $cass->percentage;
	}	
@endphp
							...
							<div class="table-responsive">
								<table id="" class="table table-bordered nowrap">
										...
										<tr>
											<div class="row">
												<div class="col-sm-12 text-center p-0 m-0">
													...
													<div class="row mt-10">
														<div class="col-sm-4">
															<h4>CLASS: {{ $class->classname }} {{ $class_arm->arm_name}}
															</h4>
														</div>
														<div class="col-sm-8 ">
									...
									<form method="post" action="{{ route('store_scores') }}">
														...
														<td>
															@foreach ($scores as $score)
																@if ($score->student_id == $Student->id)
																	<input type="number"
																		name="scores[{{ $Student->id }}][{{ $score->cass_type }}]"
																		style="width: 100%;" class="form-control"
																		value="{{ $score->scores }}"
																		max="{{ $cassMax[$score->cass_type] }}">
																@endif
															@endforeach
														</td>
													@endforeach
										...
										<tfoot>
											<tr>
												<th colspan="{{ count($Assessments) + 3 }}"><button type="submit"
														style="float: right;" class="btn btn-secondary">Update CASS
													</button></th>
											</tr>
										</tfoot>
								</table>
